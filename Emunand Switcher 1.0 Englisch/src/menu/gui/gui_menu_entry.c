                                        ...
                                        int (*handler)(void *), void *param)
{
    ...
    if (bitmap != NULL)
    {
        ...
        strcpy(menu_entry->text, text); // If not default icon, text is not needed on touch input_eigentlich zweiter text string orginal nur ""
    }
    else
...
/* Erstellt ein Menu Eintrag entry_text */
gui_menu_entry_t *gui_create_menu_entry_text(const char* text,
	u8* bitmap,
	u32 x, u32 y,
	u32 width, u32 height,
	int (*handler)(void*), void* param)
...

{
	...
	strcpy(menu_entry->text, text); // If not default icon, text is not needed on touch input
