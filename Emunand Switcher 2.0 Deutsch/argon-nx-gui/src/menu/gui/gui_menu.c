...
static void update_status(lv_task_t* status)
{
	...
	//! TODO: Parse time and use offset. Test eigen...
	...
	if (per1 <= 5) {
		lv_label_set_array_text(status_bar.akkusym, LV_SYMBOL_BATTERY_EMPTY" Warnung, Akku fast leer! Bitte Ladegerät anschliessen!", 64);
	}
	...
	if (f_stat("sxos/titles/0100000000001000", NULL)) {
		...
		lv_mbox_set_text(status_bar.themeinfosx, "OK kein Theme installiert!");
	...
	if (f_stat("atmosphere/titles/0100000000001000", NULL)) {
		...
		bg.body.opa = LV_OPA_0;
;
		lv_mbox_set_text(status_bar.themeinfoatm, "OK kein Theme installiert!");
		...
		lv_mbox_set_text(status_bar.themeinfoatm, "Achtung Theme installiert!\nNach einem Systemsoftware Update\neines Emunandes, muss das Theme\nentfernt werden um den Starterror\nzu vermeiden...");
		lv_mbox_set_style(status_bar.themeinfoatm, LV_MBOX_STYLE_BG, &bg);
	}
...
static bool render_title(argon_ctxt_t * ctxt)//Title und Akku, Zeit, Datum anzeige
{
	//Title Erstellen
    lv_obj_t* title = lv_label_create(lv_scr_act(), NULL);
    lv_obj_align(title, lv_scr_act(), LV_ALIGN_IN_TOP_LEFT, 15, 620);
    //lv_label_set_text(title, "Arg"LV_SYMBOL_METEOR"nnx");
	lv_label_set_text(title, "Emunand Switcher v2.0");
	...
	//Definition, Tool Akku, Zeit und Datum auslesen und anzeigen
...
static bool render_switcherSX_tab(lv_obj_t* par, argon_ctxt_t* ctxt) //Eigener Tab SwitcherSX
{
	...
	lv_obj_set_size(btn_emunand, LV_HOR_RES / 2.5, 500);
...
static bool render_switcherATM_tab(lv_obj_t* par, argon_ctxt_t* ctxt) //Eigener Tab SwitcherATM
{
	...
	lv_obj_set_size(btn_emuatm, LV_HOR_RES / 2.5, 500);
	lv_cont_set_layout(btn_emuatm, LV_LAYOUT_COL_L);//lv_cont_set_layout(btn_test, LV_LAYOUT_CENTER);//Alle Buttons ausrichtung
...
static bool render_tools_tab(lv_obj_t* par, argon_ctxt_t* ctxt)
{
	...
	lv_obj_set_pos(more_to_come, LV_HOR_RES_MAX / 2 + 130, labels_y);
	...
	lv_obj_set_pos(btn_test, LV_HOR_RES_MAX / 2 + 80, labels_y);//possition Buttons
	lv_obj_set_size(btn_test, LV_HOR_RES / 2.5, 500);
	...
	lv_obj_set_pos(power_label, 130, labels_y);
	...
	lv_obj_set_size(btn_cont, LV_HOR_RES / 2.5, 450);
