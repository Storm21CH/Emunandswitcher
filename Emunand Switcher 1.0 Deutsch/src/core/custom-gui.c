...
int screenshot(void* params)
{
    ...
    sd_save_to_file(buff, imagesize + 54, "emunandswitcher/screenshot.bmp");
    free(buff);
	free(g_gfx_ctxt.fb);
    g_gfx_con.scale = 2;
    gfx_con_setpos(&g_gfx_con, 0, 700);
    gfx_printf(&g_gfx_con, " Screenshot gespeichert! Zu finden in emunandswitcher/screenshot.bmp");
    return 0;
}